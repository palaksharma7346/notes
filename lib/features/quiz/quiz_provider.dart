...
class QuizNotifier extends StateNotifier<AsyncValue<List<QuizQuestion>>> {
  ...
  Future<void> load({bool force = false}) async {
    ...
    final cached = mapListFromDynamic(session.generatedContent[FeatureKeys.quiz]);
    if (!force && cached.isNotEmpty) {
      state = AsyncValue.data(
        cached.map((json) => QuizQuestion.fromJson(json)).toList(growable: false),
      );
      return;
    }
    ...
    try {
      final raw = await GeminiService.generateQuiz(
        notesForSession(session),
        images: imagesForSession(session),
      );
      await ref.read(sessionsProvider.notifier).updateFeature(sessionId, FeatureKeys.quiz, raw);
      state = AsyncValue.data(
